...
def main(argv):
  ...
  parser.add_option('--emailToAdd',
                     help='Email address of user to add to group')
  ...
  if (options.whoCanInvite or options.whoCanJoin or options.whoCanPostMessage
      or options.whoCanPostMessage or options.whoCanViewMembership or options.emailToAdd) is None:
